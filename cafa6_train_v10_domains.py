...
# -*- coding: utf-8 -*-
"""
CAFA-6 Multi-Ontology Training - V10 WITH DOMAIN FEATURES
=================================================================================
...
def train_all():
    ...
    for aspect in ASPECTS:
        ...
        results.append({'aspect': aspect, 'name': name, 'fmax': fmax, 'threshold': threshold, 'classes': num_classes})
    ...
    print(f"\n{'='*80}\nSUMMARY\n{'='*80}")
    for r in results:
        print(f"{r['aspect']:<4} {r['name']:<25} Fmax={r['fmax']:.4f} t={r['threshold']:.2f} classes={r['classes']}")
    print(f"AVG Fmax: {np.mean([r['fmax'] for r in results]):.4f}")
